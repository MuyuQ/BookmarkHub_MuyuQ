...
**版本**: 0.7 | **更新日期**: 2026-07-11 | **分支**: main
...
| **手动同步传输** | `manualSyncTransfer.ts` | 手动上传/下载逻辑，支持 Gist 和 WebDAV |
| **设置管理** | `setting.ts` | 用户配置获取、缓存管理 |
| **加密模块** | `crypto.ts` | AES-GCM 凭证加密、PBKDF2 密钥派生 |
| **设置存储** | `optionsStorage.ts` | 设置持久化、敏感字段自动加解密 |
...
⚠️ **注意事项**:
- GitHub Token 和 WebDAV 密码使用 AES-GCM 加密存储（`crypto.ts`）
- 用户可设置主密码（masterPassword）增强加密强度，否则使用扩展 ID 派生密钥
- 主密码本身也加密存储在浏览器中
- WebDAV 使用 Basic Auth（Base64 编码，传输层非加密）

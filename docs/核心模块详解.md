...
| `uploadBookmarks()` | 上传合并后的数据（通过 `manualSyncTransfer`） |
| `saveSyncState()` | 持久化同步状态（进入同步时立即调用） |
| `clearSyncState()` | 清除持久化的同步状态 |
...
export async function performSync(): Promise<SyncResult> {
    ...
    await saveSyncState();  // 立即持久化，防止 MV3 休眠丢失状态
...
## 7. 设置管理 (setting.ts & optionsStorage.ts)
...
### 7.3 敏感字段加密 (optionsStorage.ts)

敏感字段使用 AES-GCM 自动加密存储：

| 字段 | 类型 | 加密方式 |
|------|------|----------|
| `githubToken` | 凭证 | 主密码加密（有主密码时）或扩展 ID 加密 |
| `webdavPassword` | 凭证 | 同上 |
| `masterPassword` | 主密码 | 始终使用扩展 ID 加密 |
...
**加密流程**:
1. 用户保存设置 → `setEncrypted()` 被调用
2. 验证输入 → `validateOptions()` 检查 URL 格式、必填字段
3. 解密旧凭证 → 使用旧主密码（或存储中的主密码）解密现有凭证
4. 加密新凭证 → 使用新主密码加密所有明文凭证
5. 加密主密码 → 主密码本身也加密存储
6. 写入存储 → `optionsStorage.set()`

**解密流程**:
1. 读取设置 → `getAllDecrypted()` 被调用
2. 解密主密码 → `decryptStoredMasterPassword()`
3. 遍历凭证字段 → 使用主密码解密（或回退到扩展 ID）
4. 解密失败 → 置空字段并记录日志
...
使用 Web Crypto API 实现 AES-256-GCM 加密，密钥通过 PBKDF2 从密码派生。
...
| IV 长度 | 12 字节 | GCM 标准 |
| Salt 长度 | 16 字节 | 防彩虹表 |
| PBKDF2 迭代 | 600,000 | OWASP 2023 推荐 |
...
function getEncryptionPassword(masterPassword?: string): string {
    ...
    // 回退到扩展 ID（向后兼容）
    return `BookmarkHub-${browser.runtime.id}-encryption-key`;
}
```

---

## 13. 手动同步传输 (manualSyncTransfer.ts)

### 13.1 模块职责

封装手动上传/下载书签的逻辑，支持 GitHub Gist 和 WebDAV 两种后端。被 `background.ts` 的 `upload`/`download` 消息处理器调用。

### 13.2 核心函数

```typescript
/** 手动上传书签到选定后端 */
export async function uploadManualBookmarks(
    setting: Setting,
    bookmarks: BookmarkInfo[]
): Promise<SyncData>

/** 从选定后端下载书签 */
export async function downloadManualBookmarks(
    setting: Setting
...
```typescript
// uploadBookmarks() 在 background.ts 中的调用
async function uploadBookmarks() {
    const setting = await Setting.build();
    const bookmarks = await getBookmarks();
    // 委托给 manualSyncTransfer 处理后端选择和数据格式化
